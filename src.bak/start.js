...
const {get} = require('../src/info');

// 公用模版
const commonTpl = require(path.join(projectPath, './build/template.js'));
const htmlConfig = get('template-entry');
...
module.exports = (env, config) => {
	...
	if(process.env.NODE_ENV === 'prod') {
		compiler.run(handleCallback(config));
	}
	else {
		compiler.watch({
...
// 无论生产环境还是开发环境，都会执行这个
function handleCallback(webpackConfig) {
